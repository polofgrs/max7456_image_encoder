...
void setup()
{
  ...
  //osd_create_char();  // Run this once only as data is stored in eeprom
  delay(500);
  ...
  //osd_print_byte(3,24,0xC0); //for the custon character (change the address)
...
void osd_print_byte(unsigned int osd_line, unsigned int osd_col, const byte s)
{
  osd_buffer[((osd_line-1)*MAX_SCREEN_WIDTH)+osd_col] = s;
}
